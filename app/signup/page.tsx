...
export default function SignupPage() {
  ...
  async function completeSignup(favoriteTeamSlug: string | null) {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch("/api/auth/signup", {
        ...
        body: JSON.stringify({ username, password, favoriteTeamSlug }),
      });
  ...
  async function handleTeamPick(team: NbaTeam) {
    await completeSignup(team.slug);
  }
        ...
        {step === 1 ? (
          <>
            ...
            <form onSubmit={handleStep1Submit} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                className="mt-2 flex h-12 w-full items-center justify-center rounded-full bg-foreground px-5 text-background transition-colors hover:bg-[#383838] dark:hover:bg-[#ccc]"
              >
                다음
              </button>
            </form>
          ...
          <>
            <div className="flex flex-col gap-1 text-center">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                선택한 팀의 컬러로 사이트 테마가 바뀝니다.
              </p>
            </div>

            {error && <p className="text-center text-sm text-red-600 dark:text-red-400">{error}</p>}

            <TeamPicker variant="full" onPick={handleTeamPick} onSkip={handleSkipTeam} />

            {loading && (
              <p className="text-center text-sm text-zinc-500 dark:text-zinc-400">가입 중...</p>
            )}
          </>
        )}
      </main>
